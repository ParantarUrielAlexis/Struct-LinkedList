  ...
  this.setState((prevState) => {
    ...
    if (this.checkCollision(head, snake)) {
      ...
      this.reportProgress(
        ...
        score,
        stars,
        prevState.collectedPrimes.length, // Report number of primes collected
        timeSurvived,
        true
      );
    ...
    let newScore = score;
    let newFoods = [...foods];
    let newCollectedPrimes = [...prevState.collectedPrimes];
    ...
    for (let i = 0; i < newFoods.length; i++) {
      ...
      if (head.x === food.x && head.y === food.y) {
        ...
        const isPrime = this.isPrime(food.number);

        if (isPrime) {
          eatenFoodIsPrime = true;
          newScore += 1;
          newCollectedPrimes = [...prevState.collectedPrimes, food.number];
          ...
          if (newCollectedPrimes.length > 0) newCollectedPrimes.pop();
    ...
    return {
      ...
      foods: newFoods,
      score: newScore,
      collectedPrimes: newCollectedPrimes,
